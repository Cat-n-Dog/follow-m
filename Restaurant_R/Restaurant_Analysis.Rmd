...
train_df <- train_df %>%
  dplyr::mutate(Open.Year = year(Open.Date), Open.Month = month(Open.Date),
                City2 = ifelse(test = City.Group == 'Big Cities', yes = 1, no = 0))
...
```{r}
train_tbl_df_mdl <- train_tbl_df %>% dplyr::select(Id, P1:Open.Year, City2)
mdl <- lm(formula = revenue ~ . - Id, data = train_tbl_df_mdl)
summary(mdl)
sqrt( mean( residuals(mdl) ^ 2 ) )
```
