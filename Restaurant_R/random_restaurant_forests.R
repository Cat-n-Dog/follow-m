...
train_df_wo_7_outliers <- train_df %>% filter(revenue < 1e7, !(Id %in% c(100,116,53,49)))
...
train_df_model_1 <- train_df_wo_7_outliers %>%
  select(-(Id:City.Group))
...
train_df_model_2 <- train_df_wo_7_outliers %>%
  select(-(Id:Type))
...
y.test.train_df <- train_df_wo_7_outliers[-train, 'revenue']

rf_caret <- train(revenue ~ ., data = train_df_model_1,
                 method = "rf",
...
rf_caret_1 <- train(revenue ~ ., data = train_df_model_1,
                  method = "rf",
                  trControl = fitControl)
rf_caret_2 <- train(revenue ~ ., data = train_df_model_2,
                    method = "rf",
                    trControl = fitControl)
...
submission_model_1 <- predict(rf_caret_1, newdata = select(test_df_model_1, -(Id:City.Group)))
...
submission_model_2 <- predict(rf_caret_2, newdata = select(test_df_model_2, -(Id:Type)))
...
write.csv(submission_df, file='submission_rf_caret_out7.csv', row.names=TRUE, quote=FALSE)
